...
def to_half(b: Collection[Tensor]) -> Collection[Tensor]:
    ...
    return recurse(
        lambda x: (
            x.half() if x.dtype not in [torch.int64, torch.int32, torch.int16] else x
        ),
        b,
    )
...
def to_float(b: Collection[Tensor]) -> Collection[Tensor]:
    ...
    return recurse(
        lambda x: (
            x.float() if x.dtype not in [torch.int64, torch.int32, torch.int16] else x
        ),
        b,
    )
...
def model_type(dtype):
    ...
    return (
        ...
        else torch.int64 if np.issubdtype(dtype, np.integer) else None
    )
